...
- Test contract: `902` total tests. The `2026-08-20` Linux runs passed all `902` (897 after the P1-1 protocol guards, 902 after the P1-2 error-visibility fixes); the earlier Windows run passed `898` with the four failures being the platform-dependent desktop adapter tests when BurntToast is unavailable. Release documentation retains `902` as the full contract count.
