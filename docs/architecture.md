...
- QQ single-chat native buttons and QQ group text fallback are contract-tested only; QQ/WeChat iLink/DingTalk image envelopes are wired and contract-tested, while real provider payload/device behavior remains unverified. The Web/admin surface has no secure `ask_user` settlement entry.
