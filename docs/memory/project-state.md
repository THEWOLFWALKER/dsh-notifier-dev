...
Snapshot date: 2026-08-25 (maintenance-architecture line, maintenance batch 6-C done; Control Core step 1 chat-level evidence gate implemented on `codex/questions-chat-gate`).
...
- Branch topology (2026-08-25, re-verified against git): private `main` = `37f2ec7` — the v0.8.6 release (`bf03a1c`) plus the crack-fix v0.8.7 batches A/B/C (contract 1012); all relay lines (security-hardening, protocol-guards, error-visibility, P1-3, crack-fix) are contained in `main` and retired. Active line: `codex/maintenance-architecture` (unmerged, unreleased, `main` + 23 commits, fast-forward-mergeable) — maintenance batches 1..5 + 6-A/6-B/6-C complete, contract **1111** (1110 pass + 1 win32 skip) at HEAD `9ae636b` (docs closure `2a59ed5`). Batch 5 added `src/inbound/message.mjs` unified inbound text/image/file structure + `parseQQImageMessage` interface (NOT wired into qq-gw — no protocol evidence yet). Batch 6-A (Issue #10) reworked Dashboard onboarding; 6-B pinned QQ RESUME/ACK behavior; 6-C scoped question escalation reminders to exact inbound targets and rejected failed hint delivery. Plan/workstream in `.agents/workstreams/maintenance-architecture.md`. Residual-test branch `codex/hint-scope-regression` (2026-08-25) pinned the channel-level hint/evidence ambiguity with 3 regression tests (contract 1111 → 1114); behavior unchanged.
- Control Core step 1 — chat-level evidence gate, branch `codex/questions-chat-gate` (2026-08-25, branched from `codex/hint-scope-regression` HEAD `7146f0f`): implements `hintTargets` per-target `(channel, chatId, userId)` evidence tracking, `(channel, userId, chatId)` triple matching (`exact ?? hint ?? chatMismatch`), incremental persistence (`persistHints`), and zombie-row filtering via `bus.hasWaiter`. Flips the 3 residual pins to rejection semantics; adds 6 chat-gate regression tests + 1 upgrade-reminder invariance test + 1 `hasWaiter` full-exit contract test. P1 residual in `docs/memory/risks.md` closed; new known limitation registered (upgrade-reminder targets with failed话术 delivery lack hintTargets evidence). Contract **1122** (Linux host: 1122 pass, 0 skip). Uncommitted workspace on this branch — not yet pushed.
...
- Control Core branch `codex/questions-chat-gate` (2026-08-25): `node --test test/*.test.mjs test/*.spec.mjs` reports **1122** tests (Linux host: 1122 pass, 0 fail, 0 skip) — the 3 residual pins flipped to rejection semantics + 8 new regression tests (chat-gate ×6, upgrade-reminder invariance ×1, `hasWaiter` full-exit contract ×1). P1 residual in `docs/memory/risks.md` closed.
