...
On Windows without the BurntToast PowerShell module, the full suite is expected to report four failures in `test/desktop.test.mjs`; treat this as a desktop capability gap and validate that adapter on a host with BurntToast installed. The remaining `887` tests must pass for the current `891`-test contract.
