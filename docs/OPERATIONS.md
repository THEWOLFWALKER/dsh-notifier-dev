...
The current development baseline is `1349` tests (`1348` pass + `1` skip). The published v0.8.6 contract remains `909`; do not change `package.json`'s release count on an unreleased development line. Desktop/host and provider protocol behavior still require real-device validation where noted below.
