...
The current development baseline is `1177` tests (`1176` pass + `1` skip). The published v0.8.6 contract remains `909`; do not change `package.json`'s release count on an unreleased maintenance line. Desktop/host and provider protocol behavior still require real-device validation where noted below.
