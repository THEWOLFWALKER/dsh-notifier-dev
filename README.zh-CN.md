...
已发布的 `dsh-notifier@0.8.6` 契约为 909 个测试；当前开发线为 1346 个（1345 通过 + 1 个跳过），尚未发布，也未做真实设备/宿主协议验证。
...
test/                 909 个测试（已发布 v0.8.6 包）；当前开发线 1346 个测试（1345 通过 + 1 个跳过）
```
...
```bash
npm test          # 已发布 v0.8.6 契约：909 个用例；当前开发线 1346（1345 通过 + 1 个跳过）
```
