...
- 2026-08-27 协议预审落地之后的渠道适配安全收尾（`docs/protocol-preflight/` 为事实边界，不改协议猜测；全部 mock/contract 证据，无真机验证）。聚焦 + 全量测试通过：**npm test = 1339（1338 pass + 1 skip）**，`verify-release`（909）/`gen-channel-matrix --check`（27 渠道）/`node --check src/index.mjs`/`git diff --check` 全绿。修掉的 7 个失败为 HEAD 预存（phase-1 源码硬化后的测试断线），本次一并修复：
