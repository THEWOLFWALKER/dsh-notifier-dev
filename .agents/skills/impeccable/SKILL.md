---
name: impeccable
description: Use when the user wants to design, redesign, shape, critique, audit, polish, clarify, distill, harden, optimize, adapt, animate, colorize, extract, or otherwise improve a frontend interface. Covers websites, landing pages, dashboards, product UI, app shells, components, forms, settings, onboarding, and empty states. Handles UX review, visual hierarchy, information architecture, cognitive load, accessibility, performance, responsive behavior, theming, anti-patterns, typography, fonts, spacing, layout, alignment, color, motion, micro-interactions, UX copy, error states, edge cases, i18n, and reusable design systems or tokens. Also use for bland designs that need to become bolder or more delightful, loud designs that should become quieter, live browser iteration on UI elements, or ambitious visual effects that should feel technically extraordinary. Not for backend-only or non-UI tasks. 中文触发：设计/重设计/打磨/审查/润色/简化/强化前端界面、消除 AI 模板味时使用。
version: 4.3.1 (self-contained snapshot in dsh-notifier)
license: Apache 2.0
# --- 本地自包含适配备注（dsh-notifier dev）---
# 官方 skill 依赖自编译二进制 CLI（`npx impeccable` / `scripts/impeccable`），本仓库不携带该运行时。
# 因此在 dsh-notifier 内使用本 skill 时：把下方文档中的 `impeccable <verb> <target>` CLI 调用
# 降级为「直接阅读并按其指令执行对应 `reference/<verb>.md`」——reference/ 39 个纯文本即该 CLI 的全部设计指令，
# 不装二进制也能完整落地（craft / audit / critique / bolder / quieter / polish / distill / harden 等）。
# 唯一例外：`live` / 浏览器逐元素迭代需要一个真实浏览器会话，本仓库用 Playwright 脚本代替。
# 官方完整仓库：github.com/pbakaus/impeccable。以下为原文档（仅顶部说明为本仓库附加）。
---
