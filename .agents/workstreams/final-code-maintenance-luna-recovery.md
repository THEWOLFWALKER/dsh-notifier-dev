...
- Focused tests: overlay/SDK/provider suites 128/128 pass; `node --test test/admin-ui-behavior.test.mjs` 31/31 pass after destructive pairing-code revoke confirmation.
- Full validation rerun: `npm test` 1349 total (1348 pass + 1 skip); `verify-release` passed (v0.8.6 / 909), channel matrix passed (27), `node --check src/index.mjs`, `git diff --check`, and `npm pack --dry-run --json` passed (193 package entries, no bundled dependencies).
