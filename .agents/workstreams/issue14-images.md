...
- Handoff: Issue #14 is code-level complete and contract-tested for QQ C2C `extra`, iLink item_list, and DingTalk picture/image URL shapes. Real provider payload variants, signed/media-id download semantics, and on-device rendering remain unverified; no real-device claim is made. End-of-task documentation audit found stale statements in `docs/architecture.md`, `docs/memory/risks.md`, `HANDOFF.md`, and `CHANGELOG.md`; the assigned scope explicitly forbids touching them, so the parent must reconcile them before release. Commit `6871f0f`.
