...
- 代码量：src+test+scripts ≈ 36,000 行；47 个测试文件，1339 测试（1338 pass + 1 skip；已发布 v0.8.6 = 909）
...
| 测试 | `npm test` 已发布 v0.8.6 契约 = **909 tests**；当前开发线 = **1339**（1338 pass + 1 skip；含本次协议适配安全收尾与阶段 1-6 硬化测试：wxpusher 本地 accountId、渠道 accountId 来源绑定、按钮 eventId、overlay→Control Core 集成、PR#12 硬化、跨进程写入、渠道 fail-closed、个人 UX admin API、安全/结构边界） |
...
| 测试 | `test/` | 行为契约随包分发是项目惯例（已发布 v0.8.6 = 909 用例；开发线 HEAD 1339，装包即可 `npm test`） |
...
```bash
npm test                    # 当前开发线 1339（1338 pass + 1 skip）；已发布 v0.8.6 契约仍为 909
