...
| 测试 | `npm test` 已发布 v0.8.6 契约 = **909 tests**；当前开发线 = **1329**（1328 pass + 1 skip；含阶段 1-6 硬化测试：overlay→Control Core 集成、PR#12 硬化、跨进程写入、渠道 fail-closed、个人 UX admin API、安全/结构边界） |
...
| 测试 | `test/` | 行为契约随包分发是项目惯例（已发布 v0.8.6 = 909 用例；开发线 HEAD 1230，装包即可 `npm test`） |
...
```bash
npm test                    # 当前开发线 1329（128 pass + 1 skip）；已发布 v0.8.6 契约仍为 909
