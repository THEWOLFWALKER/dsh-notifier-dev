...
> 快照刷新：2026-08-25，维护批 1..5 + 6-A/6-B/6-C 完成；Control Core Step 1（CC-1）补齐提问编号回复的 `(channel,userId,chatId)` 隔离，测试迁移提交 `2c39cba`。`notifyAll().delivered` 仍只有渠道级证据，不能推导具体 chat 送达；未获逐目标 `sendText` 确认的编号兜底保持 fail-closed。PR #12 仅作参考，QQ 原生按钮、提问卡片与 `approval.parallel` 均等待 CC-2/CC-3 后重写，绝不整体 cherry-pick。公共仓库 issue 快照：#16/#15/#14/#13/#10 等仍开放；私有库是唯一开发基线，公共库今晚不 push/merge/release。版本串停在已发布 v0.8.6 / 909，发版轮再统一提。
