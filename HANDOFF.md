...
- 代码量：src+test+scripts ≈ 36,000 行；50 个测试文件，1352 测试（1351 pass + 1 skip；已发布 v0.8.6 = 909）
...
| 测试 | `npm test` 已发布 v0.8.6 契约 = **909 tests**；当前开发线 = **1352**（1351 pass + 1 skip；含协议适配安全收尾、阶段 1-6 硬化以及 A3/A4/A5/A6 维护） |
...
| 测试 | `test/` | 行为契约随包分发是项目惯例（已发布 v0.8.6 = 909 用例；开发线 HEAD 1352，装包即可 `npm test`） |
...
```bash
npm test                    # 当前开发线 1352（1351 pass + 1 skip）；已发布 v0.8.6 契约仍为 909
