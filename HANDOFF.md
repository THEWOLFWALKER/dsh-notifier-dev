...
- **破甲修复线 `crack-fix-batchA-v0.8.7`（2026-08-23 起，快照刷新 2026-08-24 neat 轮；2026-08-25 接手 agent 依 git 核正）**：破甲修复分批推进，**已并入私有 `main`（main = `37f2ec7`，快进）但未发版**，故四处版本/计数串仍停在已发布的 v0.8.6 / 909 契约（发版轮再统一提到 v0.8.7）。
  ...
  - 已提交并入 main（`423f334`，2026-08-25 接手 agent 依 git 核正——原文「工作区未提交」为过期快照）：批次 C1（TG/飞书来源比对 fail-closed，994→1001）、C2（审批桥 `liveWaiters` 僵尸行闸门，1001→1004）、C3（复合键冒号截断，1004→1007）、REVIEW-ABC 独立审查的 6 个 bug 修复（1007→**1012**）。改动落在 `src/actions.mjs`、`src/admin/api.mjs`、`src/approval/router.mjs`、`src/inbound/{telegram-bot,feishu-bot,identity,wxpusher-callback}.mjs` 与 8 份对应测试。
