...
更新时间：2026-08-27。当前发布线是 `codex/stage5-wechat-ilink-hardening`，包版本字段为 `0.9.0`。公共 GitHub 源码镜像已推送，npm `latest` 已发布为 `0.9.0`；公开仓库描述与发现主题已同步，Awesome DSH 更新已提交 PR #3490，dshfind 等待下一轮自动同步。最新完整测试为 `1352`（1351 pass + 1 skip）；历史 npm `0.8.6` 契约为 909。

| 测试 | `npm test`：**1352 tests**（1351 pass + 1 skip，v0.9.0 发布线） |
