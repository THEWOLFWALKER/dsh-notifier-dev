...
更新时间：2026-08-28。当前发布线是 R1 修复列车（`main`，v0.9.1）：按 2026-08 审查线 80 项问题清单完成第一批 18 项修复（W1 钉钉 Stream 协议 G-01/02/10/23/24/42、W2 码点分段 G-03/22/40、W3 命令矩阵 G-04/06/25/33/43/52/65、W4 管理台审计 G-05/41、W5 合并窗路由键 G-51/48/49），五个批次 16 条窄提交线性合入。全部为 mock/contract 证据，协议类修复未经真机验证（缺口记 `docs/memory/risks.md`）。后续列车：R2 投递与凭证可靠性（W6/W7/W8，0.9.2）→ R3 安全加固（W9，0.9.3）→ R4 配置生命周期（W10/11/12，0.9.4）→ R5 测试文档（W13，0.9.5）。

| 测试 | `npm test`：**1414 tests**（1414 pass，v0.9.1 发布线） |
