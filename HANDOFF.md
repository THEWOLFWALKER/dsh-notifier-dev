...
> 当前快照：2026-08-26，当前开发线 HEAD 为 `36aca9f`（团队模式批准权限契约：有界 `approvalMembers` 归一 + 纯 `canSettleApproval` 接入 Control Core）。当前开发线 `npm test` = 1200（1199 pass + 1 skip）；已发布 v0.8.6 仍为 909，当前线未发布。QQ C2C 单聊原生按钮、GROUP 文本回退，以及 QQ/微信 iLink/钉钉图片代码已接线并通过契约测试；缺失 `chatType` 或未知来源控制 fail-closed，`conversation` 的 `routeUnsafe` 旁路已堵。Web/admin 已具备阶段 2A 本地管理台的远程提问裁决入口（见下方快照），desktop 端仍无 settlement 入口，双端共享未宣称；真实设备/宿主协议仍未验证，能力只能标记 declared/contract-tested。
