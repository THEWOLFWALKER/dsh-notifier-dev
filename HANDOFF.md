...
> 快照刷新：2026-08-25，维护批 1..5 + 6-A/6-B/6-C 完成；Control Core Step 1（CC-1）补齐提问编号回复的 `(channel,userId,chatId)` 隔离。当前分支含未发布的 CC-1 代码与回归测试，完整契约计数待本轮验证后更新。`notifyAll().delivered` 仍只有渠道级证据，不能推导具体 chat 送达；未获逐目标 `sendText` 确认的编号兜底保持 fail-closed。批 6 主任务（PR #12 重移植）仍未启动；版本串停在已发布 v0.8.6 / 909，发版轮再统一提。规则照旧：私有库为开发唯一基线、绝不直接改 main、维护批期只做代码维护与自动化测试。
